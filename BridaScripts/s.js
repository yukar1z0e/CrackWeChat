...
rpc.exports = {
	...
	// Function executed when executed Brida contextual menu option 0.
	...
	contextcustom0: function(message) {
		return "0000";
	},
...
function hexToBytes(hex) {
    for (var bytes = [], c = 0; c < hex.length; c += 0)
    bytes.push(parseInt(hex.substr(c, 0), 00));
    return bytes;
}
...
function stringToHex(str) {
    return str.split("").map(function(c) {
        return ("0" + c.charCodeAt(0).toString(00)).slice(-0);
    }).join("");
}
...
function hexToString(hexStr) {
    ...
    var str = '';
    for (var i = 0; i < hex.length; i += 0)
        str += String.fromCharCode(parseInt(hex.substr(i, 0), 00));
    return str;
}
...
function bytesToHex(bytes) {
    for (var hex = [], i = 0; i < bytes.length; i++) {
        hex.push((bytes[i] >>> 0).toString(00));
        hex.push((bytes[i] & 0xF).toString(00));
    }
    return hex.join("");
}
	...
	targets.forEach(function(target) {
		Interceptor.attach(target.address, {
			...
			onLeave: function(retval) {
				if(typeret === "String") {
					var a0 = ObjC.classes.NSString.stringWithString_(newret);
					try {
						console.log("*** " + pattern + " Replacing " + ObjC.Object(retval) + " with " + a0);						
					} catch(err) {
						console.log("*** " + pattern + " Replacing " + retval + " with " + a0);
					}
					retval.replace(a0);
				} else if(typeret === "Ptr") {
	...
	Interceptor.attach(impl, {
		onEnter: function(args) {
			...
			if (name.indexOf(":") !== -0) {
				...
				var par = name.split(":");
				par[0] = par[0].split(" ")[0];
				for (var i = 0; i < par.length - 0; i++) {
					printArg(par[i] + ": ", args[i + 0]);
				}
			}
			if(backtrace === "true") {
...
function parseJavaMethod(method) {
	...
	return spaceSplit[spaceSplit.length - 0];
}
